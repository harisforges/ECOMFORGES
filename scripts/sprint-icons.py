#!/usr/bin/env python3
"""
Derive ForgeSprint's home-screen icons from the calculator's.

Fifth tool, fifth icon. The calculator is cyan, the analyst amber, LeadForge green and ForgeMarketing red
and ForgeSprint red — four EcomForges apps sitting next to each other on a phone
...
Violet #A855F7 is the one hue left with real separation: cyan, amber, green and red
already occupy the rest of the wheel, and a blue would sit too close to the navy ground
every icon shares.

    python3 scripts/sprint-icons.py       # writes into the repo root
...
NAVY = (22, 40, 64)
RAMP_DARK = (59, 7, 100)
RAMP_BRIGHT = (168, 85, 247)

# Each calculator icon and the ForgeSprint icon derived from it.
PAIRS = [
    ('icon-192.png', 'sprint-icon-192.png'),
    ('icon-512.png', 'sprint-icon-512.png'),
    ('icon-maskable-512.png', 'sprint-icon-maskable-512.png'),
    ('apple-touch-icon.png', 'sprint-apple-touch-icon.png'),
]

# The browser-tab favicon. Without it ForgeSprint would carry the calculator's cyan
# mark and the two tabs would be indistinguishable.
FAVICON = ('sprint-icon-192.png', 'sprint-favicon.png', 64)
